...
export function createComplianceIso27001Config(projectName: string): K6sConfig {
  return K6sConfigSchema.parse({
    version: '1',
    project: {
      name: projectName,
      description: 'ISO 27001 oriented controls with asset classification.',
    },
    session: {
    ...
    boundaries: [
      {
        ...
        max_tool_calls_per_session: 180,
      },
    ],
    gates: [
      {
    ...
    classifications: [
      {
        level: 'public',
        paths: ['README.md', 'docs/public/**', 'public/**'],
      },
      {
        level: 'internal',
      ...
      {
        level: 'confidential',
        paths: ['**/auth/**', '**/security/**', '**/config/**', '**/secrets/**', '.env*'],
      },
      {
        level: 'restricted',
