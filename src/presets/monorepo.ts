...
export function createMonorepoConfig(projectName: string): K6sConfig {
  return K6sConfigSchema.parse({
    version: '1',
    project: {
      name: projectName,
      description: 'Monorepo boundary template for package-level isolation.',
    },
    session: {
    ...
    boundaries: [
      ...
      {
        pattern: 'payments-*',
        allowed_paths: ['packages/payments/**', 'packages/shared-types/**'],
        forbidden_paths: ['.env*', '**/*.pem', '**/*.key'],
        enforcement: 'advisory',
      },
      {
        pattern: 'frontend-*',
        allowed_paths: ['apps/web/**', 'packages/shared-types/**'],
        forbidden_paths: ['.env*', '**/*.pem', '**/*.key'],
        enforcement: 'advisory',
      },
      {
