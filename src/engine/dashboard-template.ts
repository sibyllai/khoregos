...
(function() {
  ...
  var sparklineBuckets = new Array(30).fill(0);
  ...
  function buildRowElement(ev) {
    ...
    var files = Array.isArray(ev.files_affected) ? ev.files_affected.join(', ') : '';
    ...
    var tdTime = addCell(time, 'col-timestamp');
    ...
    var tdAction = addCell(truncStr(ev.action || '', 80));
  ...
  var storedTheme = localStorage.getItem('k6s-theme');
  ...
  function updateColumns() {
    var showTime = document.getElementById('colTimestamp').checked;
    var showAgent = document.getElementById('colAgent').checked;
    var showFiles = document.getElementById('colFiles').checked;
    document.querySelectorAll('.col-timestamp').forEach(function(el) { el.classList.toggle('hidden', !showTime); });
    document.querySelectorAll('.col-agent').forEach(function(el) { el.classList.toggle('hidden', !showAgent); });
    document.querySelectorAll('.col-files').forEach(function(el) { el.classList.toggle('hidden', !showFiles); });
    localStorage.setItem('k6s-cols', JSON.stringify({ time: showTime, agent: showAgent, files: showFiles }));
  }
  var savedCols = localStorage.getItem('k6s-cols');
  if (savedCols) {
    try {
      var c = JSON.parse(savedCols);
  ...
  var scrollResume = document.getElementById('scrollResume');
  ...
  auditPanel.addEventListener('scroll', function() {
    var atBottom = auditPanel.scrollHeight - auditPanel.scrollTop - auditPanel.clientHeight < 50;
  ...
  function matchesFilters(ev) {
    ...
    if (searchInput.value) {
      ...
      var text = (ev.action || '') + ' ' + (ev.event_type || '') + ' ' + JSON.stringify(ev.details || {});
  ...
  [filterType, filterSeverity, filterAgent, searchInput].forEach(function(el) {
  ...
  function renderTable() {
    ...
    updateColumns();
    var countText = filtered.length + ' events';
  ...
  function addEvent(ev) {
    if (allEvents.some(function(e) { return e.id && e.id === ev.id; })) return;
    allEvents.push(ev);

    var bucketIdx = Math.floor((Date.now() - bucketStart) / 60000);
    ...
    if (matchesFilters(ev)) {
      var tbody = document.getElementById('eventBody');
      tbody.appendChild(buildRowElement(ev));
      updateColumns();
      var filtered = getFilteredEvents();
      var countText = filtered.length + ' events';
      if (filtered.length !== allEvents.length) countText += ' (filtered from ' + allEvents.length + ')';
  ...
  function renderSparkline() {
    var container = document.getElementById('sparkline');
    var svg = container.querySelector('svg');
    var max = Math.max(1, Math.max.apply(null, sparklineBuckets));
    var w = svg.clientWidth || 600;
    var h = 40;
    var step = w / 29;
    var pathD = 'M0,' + (h - sparklineBuckets[0] / max * h);
    var areaD = 'M0,' + h + ' L0,' + (h - sparklineBuckets[0] / max * h);
    for (var i = 1; i < 30; i++) {
      var x = i * step;
      var y = h - (sparklineBuckets[i] / max * h);
      pathD += ' L' + x + ',' + y;
      areaD += ' L' + x + ',' + y;
    }
    areaD += ' L' + (29 * step) + ',' + h + ' Z';
  ...
  function connectSSE() {
    ...
    es.addEventListener('cost', function() { fetchCost(); });
    es.onerror = function() {
      document.getElementById('statusText').textContent = 'Reconnecting...';
      setTimeout(function() { document.getElementById('statusText').textContent = 'Live'; }, 3000);
    };
  }
  ...
  function fetchInitial() {
    Promise.all([
      ...
      var evData = results[0], costData = results[1], agentData = results[2], reviewData = results[3], sessData = results[4];
  ...
  function fetchCost() {
    fetch('/api/cost').then(function(r) { return r.json(); }).then(updateCostDisplay).catch(function() {});
  }

  function updateCostDisplay(data) {
    var total = Number(data.summary && data.summary.total_cost || 0);
    document.getElementById('totalCost').textContent = '$' + total.toFixed(4);
    var inp = Number(data.summary && data.summary.total_input_tokens || 0);
    var out = Number(data.summary && data.summary.total_output_tokens || 0);
  ...
  function updateReviewDisplay(items) {
    ...
    items.forEach(function(item) {
      ...
      var file = details.file || (Array.isArray(item.files_affected) ? item.files_affected[0] : '') || '';
      var div = document.createElement('div');
  ...
  document.getElementById('exportJSON').addEventListener('click', function() {
    var data = JSON.stringify(getFilteredEvents(), null, 2);
